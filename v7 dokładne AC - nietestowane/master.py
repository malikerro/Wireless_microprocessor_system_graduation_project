...
MEASURE_BUTTON_PIN = Pin(12, Pin.IN, Pin.PULL_UP) # button to turn off steering at all and freeze actual positions
...
# other global variables - not used by second thread
vol_out = 0
cur_out = 0
    ...
    try:
        ...
        while True:
            if LCD_OFF:
                ...
                if(MEASURE_OFF):
                    pwr_out = vol_out * cur_out # fastest way to calculate instant power, but not accurate
                else:
                    pwr_out = vol_out           # in this situation, vol_out and cur_out are 
                                                # actually mean of power, calculated by slave
...
while on_duty:
    ...
    POWER_OFF = POWER_BUTTON_PIN.value()
    lcd_first = LCD_BUTTON_PIN.value()
    frz_first = FREEZE_BUTTON_PIN.value()
    measure_first = MEASURE_BUTTON_PIN.value()
    ...
    controll_sum = bb_duty + lp_duty + li_duty + int(not POWER_OFF) + int(not MEASURE_OFF)
    msg_string = str(controll_sum) +'&'+ str(int(not POWER_OFF)) +'&'+ str(bb_duty) +'&'+ str(lp_duty) +'&'+ str(li_duty) +'&'+ str(int(not MEASURE_OFF))
    ...
    measure_second = POWER_BUTTON_PIN.value()
    
    if(lcd_first and not lcd_second):
        LCD_OFF = not LCD_OFF
    if(frz_first and not frz_second):
        FREEZE_OFF = not FREEZE_OFF
    if(measure_first and not measure_second):
        MEASURE_OFF = not MEASURE_OFF
